...
$equipment = [];
$sql = "SELECT * FROM equipment WHERE status = 'ດີ' ORDER BY equipment_id DESC";
...
<head>
    ...
    <style>
        ...
        .stat-banner {
            ...
            align-items: center;
            gap: 20px;
            box-shadow: 0 6px 20px rgba(17,153,142,0.20);
        }
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
        <div>
            <h4 class="fw-bold text-dark mb-1">
                <i class="fas fa-check-circle text-success me-2"></i> ອຸປະກອນດີ / ພ້ອມໃຊ້ງານ
            </h4>
            <p class="text-muted small mb-0">ລາຍຊື່ອຸປະກອນ ແລະ ເຄື່ອງຫຼິ້ນທີ່ຢູ່ໃນສະພາບດີ ພ້ອມໃຊ້ງານ</p>
        </div>
            ...
            <div class="p-3 border-bottom d-flex flex-wrap justify-content-between align-items-center gap-3">
                <div class="d-flex align-items-center flex-wrap gap-3">
                    <div class="text-muted small">
                        ສະແດງ: <span class="fw-bold text-success" id="matchCount"><?= count($equipment) ?></span> ລາຍການ
                    </div>
                    ...
                    <tbody id="eqTableBody">
                        ...
                        <tr id="emptyRow">
                            <td colspan="8" class="text-center py-5 text-muted">
                                ...
                                ຍັງບໍ່ມີຂໍ້ມູນອຸປະກອນໃນສະພາບດີ
                            </td>
                        </tr>
                        <?php else: ?>
...
function updateFilteredRows() {
    ...
    if (filteredRows.length === 0 && $('.eq-row').length > 0) {
        if ($('#emptySearch').length === 0)
            $('#eqTableBody').append('<tr id="emptySearch"><td colspan="8" class="text-center py-4 text-muted">ບໍ່ພົບຂໍ້ມູນ</td></tr>');
